...
public class NewsService {
	...
	public NewsDto getTopNewsByCategory(String category) {
		// 1.
		Page<News> newsPage = newsDao.findTopByCategoryOrderByLikecountDescViewcountDesc(category,
				PageRequest.of(0, 1));
	...
	public NewsDto getNewsById(Integer id) {
		...
		if (news.isPresent()) {
			return NewsDto.convertToDto(news.get());
		}
		// 3. news를 DTO로 변경하 리턴
		return null;
	}
	...
	public List<NewsDto> getPopularNews() {
		return newsDao.findTop6ByOrderByViewcountDesc().stream().map(NewsDto::convertToDto)
				.collect(Collectors.toList());
	}

	// 최신 뉴스 목록 가져오기
	public List<NewsDto> getLatestNews() {
		return newsDao.findTop6ByOrderByCreateDateDesc().stream().map(NewsDto::convertToDto)
				.collect(Collectors.toList());
	}

	// 관련 뉴스 가져오기
	public List<NewsDto> getRelatedNews(String category) {
		return newsDao.findTop6ByCategoryOrderByCreateDateDesc(category).stream().map(NewsDto::convertToDto)
				.collect(Collectors.toList());
	}
